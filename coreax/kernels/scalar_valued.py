...
class IndicatorKernel(ScalarValuedKernel):
    r"""
    Define a indicator kernel.

    Given :math:`\rho =` ``output_scale``, the indicator kernel is defined as
    :math:`k: \mathbb{R}^d \times \mathbb{R}^d \to \mathbb{R}`,
    :math:`k(x, y) = \begin{cases}
    \rho\;\; \text{if}\;\; x = y,\\
     0\;\; \text{otherwise}
     \end{cases}
    ...
    @override
    def compute_elementwise(self, x, y):
        return jnp.astype(jnp.array_equal(x, y), "float")
