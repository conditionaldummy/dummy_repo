...
def median_geodesic_heuristic(
    x: Union[Shaped[Array, " n d"], Shaped[Array, " n"], Shaped[Array, ""], float, int],
) -> Shaped[Array, ""]:
    """
    Compute the median kernel bandwidth according to geodesic distance.
    ...
    x = _atleast_2d_consistent(x)
    # Calculate square distances as an upper triangular matrix
    geodesic_distances = jnp.triu(pairwise(geodesic_distance)(x, x), k=1)
    # Calculate the median of the geodesic distances
    median_geodesic_distance = jnp.median(
        geodesic_distances[jnp.triu_indices_from(geodesic_distances, k=1)]
    )

    return jnp.sqrt(median_geodesic_distance / 2.0)
