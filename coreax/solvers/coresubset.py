...
from coreax.kernels import ScalarValuedKernel, TensorProductKernel
...
def _greedy_kernel_selection(
    ...
    output_size: int,
    kernel: Union[ScalarValuedKernel, TensorProductKernel],
    unique: bool,
    ...
    if isinstance(kernel, TensorProductKernel) and isinstance(dataset, SupervisedData):
        ...
        def _update_supervised_penalty(
            updated_coreset_index: Array, kernel: TensorProductKernel = kernel
        ):
            """Penalty update when dealing with supervised data."""
            return kernel.compute(
                (x, y), (x[updated_coreset_index], y[updated_coreset_index])
            )

    elif isinstance(kernel, ScalarValuedKernel) and type(dataset) is Data:
        x = dataset.data

        def _update_unsupervised_penalty(
            updated_coreset_index: Array, kernel: ScalarValuedKernel = kernel
        ):
            """Penalty update when dealing with unsupervised data."""
            return kernel.compute(x, x[updated_coreset_index])
    else:
    ...
    def _greedy_body(i: int, val: tuple[Array, Array]) -> tuple[Array, ArrayLike]:
        ...
        if isinstance(kernel, ScalarValuedKernel):
            penalty_update = _update_unsupervised_penalty(updated_coreset_index)
        else:
            penalty_update = _update_supervised_penalty(updated_coreset_index)
...
class _GenericDataKernelHerding(
    RefinementSolver[_Data, HerdingState],
    ExplicitSizeSolver,
    PaddingInvariantSolver,
):
    r"""
    ...
    :param coreset_size: The desired size of the solved coreset
    :param kernel: :class:`~coreax.kernels.ScalarValuedKernel` instance if compressing
        unsupervised :class:`~coreax.data.Data`, else
        :class:`~coreax.kernels.TensorProductKernel` instance if compressing
    ...
    def refine(
        ...
        if solver_state is None:
            dataset, bs, un = coresubset.pre_coreset_data, self.block_size, self.unroll
...
class JointKernelHerding(
    RefinementSolver[SupervisedData, HerdingState],
    ExplicitSizeSolver,
    PaddingInvariantSolver,
):
    r"""
