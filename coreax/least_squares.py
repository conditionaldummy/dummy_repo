...
class ExactSolver(RegularisedLeastSquaresSolver):
    r"""
    Base class for solving a system of well-determined linear matrix equations.

    Given an array :math:`A \in \mathbb{R}^{n \times n}`, a regularisation parameter
    :math:`\lambda \in \mathbb{R}_{\ge 0}`, and an array of targets
    :math:`B \in \mathbb{R}^{n \times m}` the exact solution to the regularised
    linear equation :math:`(A + \lambda I_n)B = X` is
    :math:`X = (A + \lambda I_n)^{-1}B` where :math:`I_n \in \mathbb{R}^{n\times n}` is
    the identity matrix.

    .. warning::
        This solver is only valid for well-determined systems of matrix equations. That
        is, the ``array`` must be square, and full rank, we do not check this!
    """
    ...
    def solve(
        ...
        return jnp.linalg.solve(
            a=array + abs(regularisation_parameter) * identity, b=target
        )
